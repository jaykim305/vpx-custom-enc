...
static void show_psnr(struct stream_state *stream, double peak) {
  ...
  fprintf(stream->log, "Stream %d PSNR (Overall/Avg/Y/U/V)", stream->index);
  ovpsnr = sse_to_psnr((double)stream->psnr_samples_total, peak,
                       (double)stream->psnr_sse_total);
  fprintf(stream->log, " %.3f", ovpsnr);

  for (i = 0; i < 4; i++) {
    fprintf(stream->log, " %.3f", stream->psnr_totals[i] / stream->psnr_count);
  }
  fprintf(stream->log, "\n");  
}
...
int main(int argc, const char **argv_) {
  ...
  char *log_filename = (char*)malloc(20*sizeof(char));
  int cpu_used, tile_col;
  for (int argidx = 0; argidx < streams->config.arg_ctrl_cnt; argidx++) {

    if (streams->config.arg_ctrls[argidx][0] == VP8E_SET_CPUUSED) {
      cpu_used = streams->config.arg_ctrls[argidx][1];
    } else if (streams->config.arg_ctrls[argidx][0] == VP9E_SET_TILE_COLUMNS) {
  ...
  for (pass = global.pass ? global.pass - 1 : 0; pass < global.passes; pass++) {
    ...
    while (frame_avail || got_data) {
      ...
      if (!global.limit || frames_in < global.limit) {
        ...
        if (!global.quiet) {
          ...
          if (fps < 30 && seen_frames > 5) goto end;
    ...
    if (!global.quiet) {
      ...
      FOREACH_STREAM(fprintf(
          stream->log,
          "Pass %d/%d frame %4d/%-4d %7" PRId64 "B %7" PRId64 "b/f %7" PRId64
