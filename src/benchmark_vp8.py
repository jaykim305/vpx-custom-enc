...
if __name__ == '__main__':
    ...
    for s in speed:
        for th in threads:
            ...
            print(f"===> benchmarking speed: {s}, thread: {th}")
            ...
            print(cmd)
            os.system(cmd)
            filesize = os.path.getsize(f"thread{th}_cpu{s}.log")
            if filesize == 0:
                os.system(f"rm thread{th}_cpu{s}.log")
    ...
    summary.write("speed thread fps\n")
    for s in speed:
        for th in threads:
            log = f"thread{th}_cpu{s}.log"
            if os.path.isfile(log):
                f = open(log, "r")
                fps = re.findall("\d+.\d+ fps", f.readlines()[0])[0][:4]
                print(fps)
                summary.write(f"{s} {th} {fps}\n")
